...
open Classical in
/-- **The role/palette factor of the aggregate deviant mass** (11A checkpoint,
round 2): summing each event's deviant fiber-pair mass over the FULL event index
carries the exact `6·K` multiplicity — six ordered role pairs times the
`K = Fintype.card (BinaryPairPalette L)` palette colors, each event bounded by the
per-color witness deviance — and is here BOUNDED by `6·K` times the per-color bound
`(δ/η²)·n²` (an inequality, not an exact evaluation).

Factor accounting for the (rejected) aggregate-cost route, on record IN PROSE:
charging each deviant event the coarse-pair volume `|C|·|D|` and dividing by the two
pinned candidate weights (each at least `|C|/2` by the half-mass theorem) would
multiply this bound by at most `4`, giving an unconditional expected-cost input
`μ ≤ 24·K·(δ/η²)·n²` for `sum_piFinset_weight_mul_eventCost_le`; conditioning on all
uniformity events under the half-budget schedule (`σ ≤ 1/2`) doubles it to
`48·K·(δ/η²)·n²` (`exists_piFinset_forall_not_mem_bad_cost_le`). These two constants
are composed from the proved generic lemmas but are NOT packaged as a single
relational theorem — the role-indexed aggregate-cost route was rejected at the
round-2 checkpoint (role-consistency failure, gate G10), so the selection-with-cost
wiring is intentionally absent. -/
