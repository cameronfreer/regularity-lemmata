...
theorem proxyPairMassWeight_mul_mass_le (w : BinaryPaletteStrongDiagWitness M sch δ P₀)
    {q : ℕ} (hFQ : w.fine ≤ w.coarse) (hq : w.fine.parts.card ≤ q)
  ...
  have hW1 := half_card_le_proxyCandidateWeight hFQ hq (proxyDevFst e)
  have hW2 := half_card_le_proxyCandidateWeight hFQ hq (proxyDevSnd e)
...
theorem expected_proxyNormalizedDeviationCost_le
    ...
    (hδ0 : 0 ≤ δ) (hFQ : w.fine ≤ w.coarse) (hq : w.fine.parts.card ≤ q) :
  ...
  have hstep := sum_piFinset_weight_mul_eventCost_le t (fun A => (A.card : ℝ))
    ...
    (fun e => proxyPairMassWeight_mul_mass_le w hFQ hq e)
