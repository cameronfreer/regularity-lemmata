...
* `exists_bucketAligned_subfamily` — the extraction: with at least
  ...
  bucket vectors of BOTH orientations: gate G-U3 in `RegularityLemmata/Graph/UniformUnion.lean`
  shows the reverse density is not determined by the forward one for a single relation —
...
(`RegularityLemmata/Graph/UniformUnion.lean`) consumes a SINGLE center `d` for EVERY ordered pair of
...
**G-U5** in `RegularityLemmata/Graph/UniformUnion.lean` exhibits the strict order, forward class `1`
