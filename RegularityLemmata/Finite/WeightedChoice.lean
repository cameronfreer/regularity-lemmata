...
Two further generic adapters sit on top, used by BOTH the forbidden and the cost
channel of the proxy selection:
...
* `sum_event_mass_le_of_weight_floor` — a uniform floor `w₀` on the coordinate weights
  turns the per-event masses of the `hbad` hypothesis into the AGGREGATE event mass
  scaled by `w₀⁻²` and the total weight. The number of events multiplies nothing: each
  event contributes only its own mass.
* `sum_piFinset_weight_mul_eventCost_le_of_weight_floor` — the two composed for the cost
  channel: an aggregate event-mass bound produces the `μ` input directly.
