...
/-- Membership in the rekeyed trace: `x ∈ traceOnParent A T ↔ (x : α) ∈ T` for `x : A`. -/
...
/-- Exact intersection-count preservation: for a subtype block `B : Finset A`,
`|B ∩ traceOnParent A T| = |B.map (Embedding.subtype _) ∩ T|`, the count of the block mapped
